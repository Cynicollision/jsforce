...
Connection.prototype.loginClientCredentials = function(callback) {
  // register refreshDelegate for session expiration
  //this._refreshDelegate = new HttpApi.SessionRefreshDelegate(this, createUsernamePasswordRefreshFn(username, password));
  if (this.oauth2 && this.oauth2.clientId && this.oauth2.clientSecret) {
    return this.loginByOAuth2ClientCredentials(callback);
  }
};
...
Connection.prototype.loginByOAuth2ClientCredentials = function(callback) {
  var self = this;
  var logger = this._logger;
  return this.oauth2.authenticateClientCredentials().then(function(res) {
    var userInfo = parseIdUrl(res.id);
    self.initialize({
      instanceUrl : res.instance_url,
      accessToken : res.access_token,
